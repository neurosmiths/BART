function [trodeLMM] = BART_colorTracking_bhf(ptID,BARTdir)
...
% [20190415]:: This takes up too muhc memory as is, I may have to
% downsample the data or fit a separate model to each frequecny band. 
...
for tt = 1:nTrials
    ...
    for ch = 1:nChans
        [tmpS(ch,:,:),Period]= basewaveERP(double(eachBalloonLFP{tt}(ch,:)),Fs,1,40,4,0);
    end
        
    % now removin the second of data on each side.
    Scell{tt} = tmpS(:,:,Fs*pre:end-Fs*post);
    tSec{tt} = linspace(0,round(size(tmpS,3)./Fs),size(tmpS,3));
    
    % trial variables.
    baloonSizeSurrogate{tt} = repmat(1:size(Scell{tt},3),size(Scell{tt},2),1);
    trialSurrogate{tt} = tt.*ones(size(Scell{tt},2),size(Scell{tt},3));
    isCTRLsurro{tt} = repmat(isCTRL(tt),1,size(Scell{tt},2));
