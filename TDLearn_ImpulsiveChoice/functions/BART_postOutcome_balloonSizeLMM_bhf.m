function [trodeLMM] = BART_postOutcome_balloonSizeLMM_bhf(ptID,BARTdir)
% BART_POSTOUTCOME_BALLOONSIZELMM_BHF fit linear model to BHF during balloon inflation
%
%   [BARTstats] = BART_postOutcome_balloonSizeLMM_bhf(ptID,nevFile) analyzes LFP data
...
outcome = ~logical(trigs(trigs==25 | trigs==26)-25);
...
for tt = 1:nTrials
    ...
    outcomeSurro{tt} = repmat(outcome(tt),1,size(HGcell{tt},2));
    postOutcomeSurro{tt} = repmat(postOutcome(tt),1,size(HGcell{tt},2));
...
if strcmp(whichModel,'each')
    ...
    outcomeVec = categorical(cell2mat(outcomeSurro));
    postOutcomeVec = cell2mat(postOutcomeSurro);
    ...
    for oc = 1:2
        switch oc
            case 1
                trialIdx = postOutcomeVec;
                testName = 'postBank';
            case 2
                trialIdx = nan(size(postOutcomeVec));
                trialIdx(~isnan(postOutcomeVec)) = ~postOutcomeVec(~isnan(postOutcomeVec));
                testName = 'postPop';
        end
        if ~exist(fullfile(BARTdir,sprintf('%s_postOutcome_balloonSizeTrackingGLMresults_%s_%sTrodesModel.mat',ptID,testName,whichModel)),'file')
            ...
            save(fullfile(BARTdir,sprintf('%s_postOutcome_balloonSizeTrackingGLMresults_%s_%sTrodesModel.mat',ptID,testName,whichModel)),'trodeLMM','-v7.3')
        else
            load(fullfile(BARTdir,sprintf('%s_postOutcome_balloonSizeTrackingGLMresults_%s_%sTrodesModel.mat',ptID,testName,whichModel)),'trodeLMM')
        end
    end
...
elseif strcmp(whichModel,'active_each')
    ...
    for oc = 1:2
        ...
        % saving model results for all electrodes
        if ~exist(fullfile(BARTdir,sprintf('%s_balloonSizeTrackingGLMresults_%s_%sTrodesModel.mat',ptID,testName,whichModel)),'file')
