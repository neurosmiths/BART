...
% spikesDir = '/media/user1/data4TB/data/socialCognition/socCogKiloSortFolder/';
...
%             line([0 0], [0 nTrials],'linestyle', '--', 'color', 'k')
            ...
            xlim([-(pre-1) (post-1)])
            hold off
            xlabel('time relative to bank/pop (s)', 'fontsize', 14);
            ylabel('firing rate (spikes/s)', 'fontsize', 14);
            set(gca, 'linewidth', 2, 'fontsize', 12);
            
            fDir = '~/Dropbox/Figs/BNST_BART/';
            fName = sprintf('%s_channel%d_unit%d_firingRates_%s.pdf',ptID,ch,un,alignName);
