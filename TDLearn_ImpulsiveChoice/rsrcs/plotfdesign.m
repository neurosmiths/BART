...
for f3= 1:nlev(3)
	...
	for j= 1:nlev(2)
		...
		if ~isempty(rows)
			...
			H= notBoxPlot(yy,x(unique(g(rows,1))),'style','sdline','jitter',0.05);   % ,0.7*bs,'sdline'
		...
		if nfac > 1
			if isnumeric(G(:,1))
				legend([H.semPtch],num2str(unique(G(:,1))));
			else
				legend([H.semPtch],unique(G(:,1)));
			end
			legend('boxoff');
		end
		hold off
